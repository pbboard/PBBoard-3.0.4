...
class DB_MySQLi implements DB_Base
{
	...
	function connect($config)
	{
		...
		foreach(array('read', 'write') as $type)
		{
			...
			foreach($connections[$type] as $single_connection)
			{
				...
				if($this->$link)
				{
					$this->connections[] = "[".strtoupper($type)."] {$single_connection['username']}@{$single_connection['hostname']} (Connected in ".$this->format_time_duration($time_spent).")";
					break;
				}
				else
				{
	...
	function explain_query($string, $qtime)
	{
		...
		if(preg_match("#^\s*select#i", $string))
		{
			...
			$this->explain .=
				"<tr>\n".
				"<td colspan=\"8\" style=\"background-color: #fff;\">Query Time: ".$this->format_time_duration($qtime)."</td>\n".
				"</tr>\n".
				"</table>\n".
				"<br />\n";
		}
		else
		{
			$this->explain .= "<table style=\"background-color: #666;\" width=\"95%\" cellpadding=\"4\" cellspacing=\"1\" align=\"center\">\n".
				...
				"</tr>\n".
				"<tr>\n".
				"<td bgcolor=\"#ffffff\">Query Time: ".$this->format_time_duration($qtime)."</td>\n".
				"</tr>\n".
				"</table>\n".
				"<br />\n";
		}
	...
	function error($string="")
	{
		if($this->error_reporting)
		{
			if(class_exists("errorHandler"))
			{
				...
				$error_handler->error(PBB_SQL, $error);
			}
			else
			{
	...
	function insert_query($table, $array)
	{
		...
		{
			if(isset($PBBoard->binary_fields[$table][$field]) && $PBBoard->binary_fields[$table][$field])
			{
...
function format_time_duration($time)
{
	...
	function escape_string($string)
	{
		...
		{
			$string = $this->validate_utf8_string($string, false);
		}
		elseif($this->db_encoding == 'utf8mb4')
		{
			$string = $this->validate_utf8_string($string);
		}
...
function validate_utf8_string($input, $allow_mb4=true, $return=true)
{
	...
	function replace_query($table, $replacements=array())
	{
		...
		{
			if(isset($PBBoard->binary_fields[$table][$column]) && $PBBoard->binary_fields[$table][$column])
			{
				if($value[0] != 'X') // Not escaped?
				{
					$value = $this->escape_binary($value);
	...
	function fetch_db_charsets()
	{
		...
		return array(
			...
			'utf8' => 'UTF-8 Unicode',
			'utf8mb4' => 'utf8mb4 Unicode (requires MySQL 5.5.3 or above)',
			'ucs2' => 'UCS-2 Unicode',
			'cp866' => 'DOS Russian',
